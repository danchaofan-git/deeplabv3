...
def create_model(aux, num_classes, pretrain=True):
    ...
    if pretrain:
        # 若权重没下载，则先从网上下载到本地上
        if not os.path.exists('./deeplabv3_resnet50_coco-cd0a2569.pth'):
            from torch.hub import load_state_dict_from_url
            load_state_dict_from_url('https://download.pytorch.org/models/deeplabv3_resnet50_coco-cd0a2569.pth', './')
        weights_dict = torch.load("./deeplabv3_resnet50_coco-cd0a2569.pth", map_location='cpu')
